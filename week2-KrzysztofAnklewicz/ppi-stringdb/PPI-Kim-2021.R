...
sub_interactionGraph <- makeproteingraph(myBaits = sub_input %>% select(Bait) %>% pull(),
                                         myPreys = sub_input %>% select(Prey) %>% pull())

sub_interactionGraph

sub_interactionGraph <- layoutGraph(sub_interactionGraph, layoutType = "neato")
...
sub_interactionGraph <- layoutGraph(sub_interactionGraph, layoutType="neato") #layoutType="fdp" is also cool, but slow, try it out!
...
mydegrees <- graph::degree(interactionGraph)
sort(mydegrees, decreasing=TRUE)

# Next we use the R hist() function to plot the histogram of this data.
hist(mydegrees) # most proteins only have 1 connections whilst other have many
...
# However, we will skip this because the subclusters are rather small.
# Generally, you will see that the enrichment analyses can find more specific functions and modules.


enrichment <- string_db$get_enrichment(clustersList_1[[1]])
head(enrichment, n=20)
write_xlsx(enrichment, "enrichment-result-cluster1.xlsx")
